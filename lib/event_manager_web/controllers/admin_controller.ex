defmodule EventManagerWeb.AdminController do
  ...
  def events(conn, _params),
    do: render(conn, "events.html", events: EventManager.Core.list_events())

  def new_event(conn, _params) do
    render(conn, "event_new.html",
      changeset: Event.changeset(%Event{}, %{}),
      speakers: EventManager.Core.list_speakers()
    )
  end
    ...
    with {:ok, event_params} <- maybe_put_event_banner(upload, event_params) do
      case EventManager.Core.create_event(event_params) do
        {:ok, _event} ->
          ...
          # Redireciona direto para o catálogo para ver o resultado
    ...
    with {:ok, event_params} <- maybe_put_event_banner(upload, event_params) do
      case EventManager.Core.update_event(event, event_params) do
        {:ok, event} ->
          ...
          |> redirect(to: ~p"/admin/events/#{event.id}/edit")
  ...
  def delete_event(conn, %{"id" => id}) do
    case EventManager.Core.delete_event(EventManager.Core.get_event!(id)) do
      ...
      {:error, :has_registrations} ->
        conn
        |> put_flash(:error, "Não é possível excluir evento com inscrições.")
        |> redirect(to: ~p"/admin/events")
    end
  end
  ...
  def publish_event(conn, %{"id" => id}) do
    case EventManager.Core.publish_event(EventManager.Core.get_event!(id)) do
      {:ok, _} ->
        ...
        |> redirect(to: ~p"/admin/events/#{id}/edit")

      {:error, _} ->
        conn
        |> put_flash(:error, "Erro ao publicar evento.")
        |> redirect(to: ~p"/admin/events/#{id}/edit")
    end
  end
  ...
  def cancel_event(conn, %{"id" => id}) do
    case EventManager.Core.cancel_event(EventManager.Core.get_event!(id)) do
      {:ok, _} ->
        conn
        |> put_flash(:info, "Evento cancelado.")
        |> redirect(to: ~p"/admin/events/#{id}/edit")

      {:error, _} ->
        conn
        |> put_flash(:error, "Erro ao cancelar evento.")
        |> redirect(to: ~p"/admin/events/#{id}/edit")
    end
  end
  ...
  def create_user(conn, %{"user" => user_params}) do
    case EventManager.Core.register_user(user_params) do
      {:ok, _user} ->
        conn |> put_flash(:info, "Usuário criado com sucesso.") |> redirect(to: ~p"/admin/users")

      {:error, _} ->
        conn |> put_flash(:error, "Erro ao criar usuário.") |> redirect(to: ~p"/admin/users/new")
    end
  end

  def edit_user(conn, %{"id" => id}),
    do: render(conn, "user_edit.html", user: EventManager.Core.get_user!(id))
  ...
  def update_user(conn, %{"id" => id, "user" => user_params}) do
    ...
    case EventManager.Core.update_user_profile(user, user_params) do
      ...
      {:error, _} ->
        render(conn, "user_edit.html", user: user)
    end
  end
  ...
  def delete_user(conn, %{"id" => id}) do
    ...
    if conn.assigns.current_user.id == user.id do
      conn
      |> put_flash(:error, "Você não pode excluir sua própria conta.")
      |> redirect(to: ~p"/admin/users")
    else
  ...
  def reports(conn, _params) do
    render(conn, "reports.html",
      ...
      monthly_stats: EventManager.Services.get_monthly_stats()
    )
  end

  def occupancy_report(conn, _params),
    do:
      render(conn, "occupancy_report.html",
        data: EventManager.Services.get_occupancy_report(limit: 20)
      )

  def participation_report(conn, _params),
    do:
      render(conn, "participation_report.html",
        by_course: EventManager.Services.get_participation_by_course(),
        by_department: EventManager.Services.get_participation_by_department()
      )
  ...
  def export_csv(conn, %{"event_id" => event_id}) do
    ...
    |> put_resp_header(
      "content-disposition",
      "attachment; filename=inscricoes-evento-#{event_id}.csv"
    )
